...
import { Announcement, Member, User } from '../types';
...
export const Announcements: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [a, m, user] = await Promise.all([
          storage.getAnnouncements(),
          storage.getMembers(),
          storage.getCurrentUser()
        ]);
        setAnnouncements(a.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
        setMembers(m);
        setCurrentUser(user);
      } finally {
        setLoading(false);
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const updated = await storage.getAnnouncements();
    setAnnouncements(updated.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
