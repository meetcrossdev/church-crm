...
export const Attendance: React.FC = () => {
  const [events, setEvents] = useState<Event[]>(storage.getEvents().sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
  const [members] = useState<Member[]>(storage.getMembers().sort((a,b) => a.lastName.localeCompare(b.lastName)));
  ...
  const handleOpenModal = (event?: Event) => {
      ...
      // Load existing attendance for that event
  ...
  const handleEventSelectionChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      // Reload attendance for newly selected event
  ...
  const handleSave = () => {
      ...
      if (eventIndex >= 0) {
          ...
          // Sync legacy count for simple displays elsewhere
          updatedEvent.attendanceCount = updatedEvent.attendeeIds.length;
          
          storage.saveEvent(updatedEvent);
          setEvents(storage.getEvents().sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
