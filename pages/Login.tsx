...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Fetch the concrete User profile object (not a Promise)
      if (authResponse.data.user) {
        const userProfile = await storage.getCurrentUser();
        
        if (userProfile) {
          // 3. Pass the resolved User object to parent state
          onLogin(userProfile);
        } else {
  ...
  const handleForgotSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      alert("Password reset functionality is currently handled by your administrator.");
      setShowForgot(false);
  };
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700 transition-colors disabled:opacity-50 flex justify-center items-center"
          >
            {loading ? (
                <>
