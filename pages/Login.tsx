...
export const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: authError } = await storage.login(email, password);
      ...
      if (data.user) {
        // Fetch the detailed profile after successful auth
        const userProfile = await storage.getCurrentUser();
        if (userProfile) {
          onLogin(userProfile);
        } else {
          setError('User logged in but profile not found.');
          setLoading(false);
        }
      }
    } catch (err: any) {
  ...
  const handleForgotSubmit = (e: React.FormEvent) => {
      e.preventDefault();
      alert("Password reset functionality requires Supabase SMTP configuration. Link would be sent here.");
      setShowForgot(false);
  };
