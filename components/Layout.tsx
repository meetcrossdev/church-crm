...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  ...
  const [settings, setSettings] = useState<ChurchSettings>(storage.getSettings());
  const navigate = useNavigate();
  const location = useLocation();

  // Refresh settings when location changes (in case settings were updated)
  useEffect(() => {
      ...
      >
        <div className="flex items-center h-16 px-6 border-b border-slate-100 justify-between">
          <div className="flex items-center space-x-3 text-indigo-600 overflow-hidden">
            ...
            <span className="text-lg font-bold tracking-tight truncate" title={settings.name}>
                {settings.name || 'Meetcross CRM'}
            </span>
          </div>
